...
updater = Updater(token="5839806750:AAHa-DvgcG_BcCswZwkvpUTRaTpC9CEcCP4")
dispatcher = updater.dispatcher

btn1 = 'Курс валют'
btn2 = 'Конвертор валют'

def startCommand(update: Update, context: CallbackContext):
    buttons = [[KeyboardButton(btn1)], [KeyboardButton(btn2)]]
    context.bot.send_message(chat_id=update.effective_chat.id, text="Добро пожаловать!", reply_markup=ReplyKeyboardMarkup(buttons))
...
def messageHandler(update: Update, context: CallbackContext):
    ...
    if btn1 in update.message.text:
        get(start).content
    if btn2 in update.message.text:
        get(start).content
